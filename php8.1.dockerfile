...
RUN apk update \
    && apk add --no-cache linux-headers gettext-dev libwebp-dev zlib-dev libxpm-dev freetype-dev libjpeg-turbo-dev libpng-dev libxml2-dev libzip-dev bzip2-dev unixodbc-dev libmemcached-dev libstdc++ openssl-dev gcc g++ make autoconf \
...
RUN apk add --update nodejs=16.17.1-r0
